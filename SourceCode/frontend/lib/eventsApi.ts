...
export interface EventRecord {
  ...
  ticketCount?: number; // Only included in organiser's own events
  organiser: {
    id: string;
    name: string;
...
export async function getEvents(category?: string): Promise<EventRecord[]> {
  const token = await getAuthToken();
  ...
  const data = await fetchWithAuth(url, init);
