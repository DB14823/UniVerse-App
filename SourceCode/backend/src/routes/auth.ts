...
const sanitizeOrganisation = (org: {
  ...
  evidenceImageUrl: org.evidenceImageUrl,
});
  ...
  try {
    ...
    if (decoded.role === "STUDENT") {
      ...
      if (student.profileImageUrl) {
        const publicId = extractPublicIdFromUrl(student.profileImageUrl);
        if (publicId) {
      ...
      // Upload new image
      const uploadResult = await uploadImage(image, "universe/profiles");
    ...
    if (decoded.role === "ORGANISATION") {
      ...
      if (org.profileImageUrl) {
        const publicId = extractPublicIdFromUrl(org.profileImageUrl);
        if (publicId) {
          await deleteImage(publicId);
        }
      }
      ...
      return res.json({
        user: sanitizeOrganisation(updatedOrg),
      });
    }
